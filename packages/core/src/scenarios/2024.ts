...
export const generateScenarios = (entrypoint: string): MeasureScenario[] => [
          ...
          await page.waitForFunction(
            (expect) => {
              const actual = Array.from(
                document.querySelectorAll<HTMLAnchorElement>("ul > li > a"),
              ).filter((_, i) => i < 10).map((el) => new URL(el.href).pathname);
          ...
          (expect) => {
            const actual = Array.from(
              document.querySelectorAll(
                "tbody tr td:nth-of-type(2) p:nth-of-type(2)",
              ),
            ).filter((_, i) => i < 10).map((el) => el.innerHTML);
  ...
  },
  {
    ...
    flow: async (page) => {
      ...
      try {
        await page.locator(
          'section[role="dialog"] > div > div:nth-child(4) > a',
        )
      ...
      try {
        await page.waitForResponse(
          (res) =>
            ...
            res.url().endsWith("/api/v1/episodes"),
        );
      } catch (err) {
